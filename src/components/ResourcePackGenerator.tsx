...
const ResourcePackGenerator: React.FC<ResourcePackGeneratorProps> = ({
  ...
  const [packIcon, setPackIcon] = useState<File | null>(null)
  ...
  const handleIconSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!file.type.startsWith('image/')) {
        onComplete?.(false, '画像ファイルを選択してください')
        return
      }

      // ファイルサイズをチェック（5MB制限）
      if (file.size > 5 * 1024 * 1024) {
        onComplete?.(false, 'ファイルサイズは5MB以下である必要があります')
        return
      }
          ...
          <Box sx={{ mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box sx={{ flex: 1 }}>
                ...
                />
                <label htmlFor="pack-icon-upload">
                  <Button
                    variant="outlined"
                    component="span"
                    startIcon={<Image />}
